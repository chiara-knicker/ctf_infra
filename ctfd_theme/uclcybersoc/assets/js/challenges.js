...
function renderSubmissionResponse(response, cb) {
    ...
    var answer_input = $("#challenge-input");
    ...
    $.get(script_root + "/api/v1/challenges", function (response) {
        ...
        for (var i = 0; i <= challenges.length - 1; i++) {
            ...
            if (user_solves.indexOf(chalinfo.id) == -1) {
                var chalbutton = $("<button class='btn challenge-button w-100 text-truncate pt-3 pb-3 mb-2' value='{0}'></button>".format(chalinfo.id));
            } else {
                var chalbutton = $("<button class='btn challenge-button solved-challenge w-100 text-truncate pt-3 pb-3 mb-2' value='{0}'><i class='fas fa-check corner-button-check'></i></button>".format(chalinfo.id));
            }
...
$('#submit-key').click(function (e) {
    submitkey($('#challenge-id').val(), $('#challenge-input').val(), $('#nonce').val())
});
